...
  const renderStars = () => {
    const stars = [];
    // Round the rating to the nearest whole number for display, default to 0 if undefined/null
...
  // Ensure product has a valid _id for the link, fallback to a placeholder
...
               {/* Certifique-se que 'discountPercentage' está sendo calculado corretamente nos dados do produto */}
...
                    -{Math.round(product.discountPercentage)}% {/* Arredondar desconto para display */}
                  </span>
                )}
               {/* Use processed boolean flags - Ajuste os nomes das propriedades conforme seu tipo Product */}
               {product.isNew && ( // Exemplo: se a propriedade for 'isNew'
                 <span className="bg-primary-600 text-white text-xs font-medium px-2 py-1 rounded">
                   Novo
                 </span>
               )}
               {product.isBestseller && ( // Exemplo: se a propriedade for 'isBestseller'
                 <span className="bg-secondary-500 text-white text-xs font-medium px-2 py-1 rounded">
                   Mais Vendido
                 </span>
               )}
                 {/* Adicionar outros selos se necessário, como "Oferta Relâmpago" */}
                 {/* Exemplo: product.isFlashSale && (<span className="...">Oferta Relâmpago</span>) */}
          </div>
...
            {/* Use product.reviewCount (frontend type), default to 0 if undefined/null */}
...
           {/* Parcelamento (se existir no seu tipo Product) */}
           {/* Exemplo: product.installments && (
              <p className="text-sm text-neutral-600 mb-3">{product.installments}</p>
            )*/}

           {/* Selo "Chegará hoje FULL" (se existir no seu tipo Product) */}
           {/* Exemplo: product.isFull && (
              <p className="text-sm font-medium text-green-600 mb-3">Chegará hoje <span className="font-bold">FULL</span></p>
            )*/}
...
                className="button-primary w-full text-center" // Certifique-se que .button-primary tem a estilização correta
              >
                Comprar
              </button>
            </div>
